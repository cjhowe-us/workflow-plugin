...
case "$cmd" in
  ...
  create)
    ...
    id=$(jq -r .id <<< "$res")
    jq -n --arg uri "gh-gist:$id" --argjson r "$res" '{uri:$uri, created:true, url:$r.html_url}'
    ;;
  update)
    uri=""; patch_path=""
  ...
  list)
    ...
    gh api /gists --paginate \
      | jq '{entries:[.[] | {uri:("gh-gist:"+.id), description, public, url:.html_url}]}'
    ;;
  lock)
