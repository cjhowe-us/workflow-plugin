...
parse_uri() {
  local u="$1"; local rest="${u#gh-issue:}"
...
case "$cmd" in
  ...
  create)
    ...
    jq -n --arg uri "gh-issue:$repo/$num" --arg url "$url" '{uri:$uri, url:$url, created:true}'
    ;;
  update)
    uri=""; patch_path=""
  ...
  list)
    ...
    gh issue list --repo "$repo" --state "$state" --json number,title,state,url \
      | jq --arg repo "$repo" '{entries:[.[] | {uri:("gh-issue:"+$repo+"/"+(.number|tostring)), title, state, url}]}'
    ;;
  lock)
    uri=""; owner=""; check=0
    while [ $# -gt 0 ]; do case "$1" in
