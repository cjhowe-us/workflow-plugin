...
case "$cmd" in
  ...
  create)
    ...
    num=$(jq -r .number <<< "$res")
    jq -n --arg uri "gh-milestone:$repo/$num" --argjson r "$res" '{uri:$uri, created:true} + $r'
    ;;
  update)
    uri=""; patch_path=""
  ...
  list)
    ...
    gh api "/repos/$repo/milestones?state=$state" \
      | jq --arg repo "$repo" '{entries:[.[] | {uri:("gh-milestone:"+$repo+"/"+(.number|tostring)), title, state, due_on, url:.html_url}]}'
    ;;
  lock|release)
