...
case "$cmd" in
  ...
  create)
    ...
    jq -n --arg uri "gh-pr:$repo/$num" --arg url "$url" '{uri:$uri, url:$url, created:true}'
    ;;
  update)
    uri=""; patch_path=""
  ...
  list)
    ...
    gh pr list --repo "$repo" --state "$state" --json number,title,state,assignees,url,baseRefName,headRefName \
      | jq --arg repo "$repo" '{entries: [.[] | {uri:("gh-pr:" + $repo + "/" + (.number|tostring)), title, state, url}]}'
    ;;
  lock)
    uri=""; owner=""; check=0
    while [ $# -gt 0 ]; do case "$1" in
